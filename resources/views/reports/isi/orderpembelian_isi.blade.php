    ...
    <div class="report-header">
        <h5>{{ _lang('Laporan Order Pembelian') }}</h5>
            ...
            <tr>
                ...
                @if ($noinv!=$report->id)
                    @php 
                        $noinv=$report->id
                    @endphp
                    <td>{{ date($date_format, strtotime($report->order_date)) }}</td>
                    <td>{{ $report->invoice_number }} 
                        @if ($report->order_status == 1)
                            <span class="badge badge-info">{{ _lang('Ordered')}}</span>
                        @elseif ($report->order_status == 2) 
                            <span class="badge badge-danger">{{ _lang('Pending') }}</span>
                        @elseif ($report->order_status == 3) 
                            <span class="badge badge-success">{{ _lang('Received') }}</span>
                        @endif
                    </td>
