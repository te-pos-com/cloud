    ...
    <table class="table table-bordered report-table">
        <thead>
            ...
            <th class="text-right">{{ _lang('Hpp') }}</th>
            <th class="text-right">{{ _lang('Harga Jual') }}</th>
            <th class="text-right">{{ _lang('Laba/Rugi') }}</th>
        </thead>
        <tbody>
            ...
            <tr>
                @php
                    ...
                    $sub_total = $sub_total + (($report->unit_cost-$report->hpp)*$report->quantity);
                @endphp
                @if ($noinv!=$report->id)
                    @php 
                        $noinv=$report->id
                    @endphp
                    <td>{{ date($date_format, strtotime($report->invoice_date)) }}</td>
                ...
                <td class="text-right">{{ decimalPlace($report->hpp, $currency) }}</td>
                <td class="text-right">{{ decimalPlace($report->unit_cost, $currency) }}</td>
                <td class="text-right">{{ decimalPlace((($report->unit_cost-$report->hpp)*$report->quantity))  }}</td>
            </tr>
            @endforeach
            @endif
        </tbody>
