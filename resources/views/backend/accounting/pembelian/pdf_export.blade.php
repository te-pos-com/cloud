...
<body>
    ...
    <div id="invoice-view" class="pdf">
        ...
        @php $currency = get_currency_symbol(get_company_option('currency', get_option('currency', 'USD'))); @endphp
            ...
            <table class="table table-bordered mt-2" id="invoice-item-table">
                ...
                <tbody>
                    ...
                    <tr id="product-{{ $item->product_id }}">
                        ...
                        <td class="text-right unit-cost">{!! strip_tags(decimalPlace($item->unit_cost, $currency)) !!}</td>
                        <td class="text-right discount">{!! strip_tags(decimalPlace($item->discount, $currency)) !!}</td>
                        <td>{!! xss_clean(object_to_tax($item->taxes, 'name')) !!}</td>
                        <td class="text-right sub-total">{!! strip_tags(decimalPlace($item->sub_total, $currency)) !!}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
